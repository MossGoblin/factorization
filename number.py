...
class Number():
    ...
    def __init__(self, value):
        self.value = value
        self.is_prime = utils.pp.isprime(value)
        if self.value == 1:
            self.ideal_factor = 0
            self.prime_factors = []
            ...
            if self.is_prime:
                ...
                self.prime_factors = utils.get_prime_factors(self.value)
            self.ideal_factor = utils.get_ideal_factor(self.value, self.prime_factors)
            self.mean_deviation = utils.get_mean_deviation(
